...
naive_conformal_prediction_aug_data <- function(x, p = 1){
  n <- length(x)
  x_train <- x[1:(n-p)]
  y_train <- x[(p+1):n]
  regData <- data.frame(x_train, y_train)
  n_candidate <- 200
  y_candidate <- seq(from=min(x), to=max(x), length=n_candidate)
  yconfidence_90 <- c()
  yconfidence_95 <- c()
  for (i in 1:n_candidate){
    regData.a <- rbind(regData,c(x[n], y_candidate[i]))
    fit_lm <- lm(y_train ~ x_train, data=regData.a)
    e_hat <- abs(fit_lm$residuals)
  
  
    e_hat_new <- e_hat[length(e_hat)]
    pi_y <- mean(e_hat <= e_hat_new)
    if(pi_y * n <= ceiling(0.9*n)){
      yconfidence_90 = c(yconfidence_90, y_candidate[i])
    } else if(pi_y*n <= ceiling(0.95*n)){
      yconfidence_95 = c(yconfidence_95, y_candidate[i])
    }
  }
...
run_simulation_parallel_naive_CP <- function(n, error_dist = c("Normal","Laplace"),
  ...
  results_df <- foreach(i = seq_len(num_sims), .combine = rbind,
                        .packages = c("VGAM","np"),
                        .export = c("naive_conformal_prediction_aug_data")) %dopar% {
                          set.seed(i)
                          total_len <- n + burn_in
                          ...
                          interval <-naive_conformal_prediction_aug_data(x_train)
...
clusterExport(cl, c("naive_conformal_prediction_aug_data"))
